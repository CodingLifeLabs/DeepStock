...
import { StockData, Theme } from '../types';
...
const RsiChart: React.FC<{ history: number[]; primaryColor: string }> = ({ history, primaryColor }) => {
  ...
  return (
    <div className="w-full h-24 mt-4 relative">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
        ...
        <polyline points={points} fill="none" stroke={primaryColor} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />
        
        {/* Current Value Dot */}
        <circle cx={lastX} cy={lastY} r="2" fill="#fff" stroke={primaryColor} strokeWidth="0.5" vectorEffect="non-scaling-stroke" />
      </svg>
...
const PriceChart: React.FC<PriceChartProps> = ({ history, currency, primaryColor }) => {
  // Calculate SMAs
  ...
  const sma5 = useMemo(() => calculateSMA(history, 5), [history]);
  const sma20 = useMemo(() => calculateSMA(history, 20), [history]);
  const sma60 = useMemo(() => calculateSMA(history, 60), [history]);
  const sma120 = useMemo(() => calculateSMA(history, 120), [history]);

  const [viewRange, setViewRange] = useState<{start: number, end: number}>({ start: 0, end: Math.max(0, history.length - 1) });
  ...
  useEffect(() => {
    setViewRange({ start: 0, end: Math.max(0, history.length - 1) });
  }, [history.length]);

  const visibleData = useMemo(() => {
    if (!history || history.length === 0) return [];
    const s = Math.max(0, Math.min(viewRange.start, history.length - 1));
    const e = Math.min(history.length - 1, Math.max(viewRange.end, s));
    return history.slice(s, e + 1);
  }, [history, viewRange]);
  ...
  }, [visibleData, sma5, sma20, sma60, sma120, viewRange.start]);
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (isDragging) {
      ...
      if (Math.abs(movePoints) > 0.2) {
        ...
        if (e >= history.length) { const diff = e - (history.length - 1); s -= diff; e -= diff; }
        if (s >= 0 && e < history.length) setViewRange({ start: s, end: e });
      }
    }
  };
  ...
  return (
    ...
    >
       {/* Reset Button */}
      {(viewRange.start > 0 || viewRange.end < history.length - 1) && (
          <button 
            onClick={(e) => { e.stopPropagation(); setViewRange({ start: 0, end: history.length - 1 }); }}
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
        ...
        {/* Main Line */}
        <path d={`M ${path} ${width},${height} 0,${height}`} fill="url(#priceGradient)" />
        <polyline points={path} fill="none" stroke={primaryColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />
        ...
        >
          ...
          <div className="flex justify-between gap-4 text-sm items-center">
             ...
             <div className="text-white font-mono">
               {visibleData[hoverIndex].price.toLocaleString()}
             </div>
          </div>
        </div>
      )}
...
const AnalysisCard: React.FC<AnalysisCardProps> = ({ data, theme }) => {
  ...
  return (
    <div className="w-full mb-8 space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Drawdown Card */}
        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 backdrop-blur-sm flex flex-col justify-between transition-colors">
          <div>
        ...
        {/* RSI Card */}
        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 backdrop-blur-sm relative overflow-hidden flex flex-col transition-colors">
          <div className="flex items-center justify-between mb-1">
            <span className="text-slate-400 text-sm font-medium">RSI (14) Trend</span>
            <Activity className={`w-5 h-5 ${isRsiHigh ? 'text-rose-500' : isRsiLow ? 'text-emerald-500' : 'text-primary'}`} />
          </div>
          ...
          <div className="flex-grow flex items-end">
             {data.rsiHistory && data.rsiHistory.length > 0 ? (
               <RsiChart history={data.rsiHistory} primaryColor={theme.colors.primary} />
             ) : (
               data.rsi !== null && (
                <div className="w-full h-1.5 bg-slate-700 mt-4 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-1000 ${isRsiHigh ? 'bg-rose-500' : isRsiLow ? 'bg-emerald-500' : 'bg-primary'}`}
      ...
      {data.priceHistory && data.priceHistory.length > 0 && (
        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 backdrop-blur-sm transition-colors">
